...
\begin{itemize}
    \item \textbf{RQ1:} What ontology design is required to effectively map unstructured requirements to structured test artifacts within a Knowledge Graph to enable semantic retrieval?
    \item \textbf{RQ2:} To what extent does an agent-orchestrated GraphRAG approach improve retrieval accuracy (measured by precision@k, recall@k, Mean Reciprocal Rank (MRR), and F1-score@k) compared to standard keyword-based and vector-only RAG methods?
...
\begin{itemize}
    ...
    \item \textbf{Evaluation Scope:} Performance evaluation is limited to offline metrics (precision@k, recall@k, MAP) and a qualitative human-in-the-loop utility study with a selected group of Ericsson engineers.

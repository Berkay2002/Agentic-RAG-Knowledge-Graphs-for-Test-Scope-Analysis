...
At their core, LLMs are built upon the \textit{transformer architecture}, which allows them to process sequences of data in parallel, capturing long-range dependencies more effectively than previous architectures. Recent trends favor sparse \textit{Mixture-of-Experts (MoE)} variants that activate only a subset of parameters per token, trading density for efficiency and longer-context/tool competence. This thesis relies on that capability rather than specific vendor offerings; proprietary (e.g., Gemini-class) and open-weight (e.g., DeepSeek-class) models both follow this trajectory, so the emphasis remains on the architectural pattern rather than the brand.
...
While generic language models excel at natural language understanding, they often \textbf{fail to capture the semantic nuance of source code} or specialized software engineering terminology. This "semantic gap" \cite{fauzan2021different} matters because it determines how well retrieval surfaces tests tied to a change request. Failure modes include:
\begin{itemize}
...
\end{itemize}
Therefore, for effective test scope analysis, the embedding model must understand code syntax and domain vocabulary; we rely on code-pretrained embeddings (e.g., CodeBERT-class \cite{guo2021codebert}) to reduce tokenization and structural errors without over-emphasizing model branding.

	extbf{Connection to Integration:} Because semantic search, even with code-specific embeddings, has blind spots (e.g., structural queries like "all functions called by class X"), the agent must have the autonomy (Section \ref{sec:integration-architecture}) to bypass pure vector search in favor of graph traversal when a query signals structural intent (Section \ref{sec:query-analysis}). This adaptive selection ensures robust retrieval across diverse query types.
...
In software repositories, sparse retrieval is indispensable for exact strings. BM25 \cite{robertson2009probabilistic, robertson1994approximations} extends TF-IDF \cite{jones1972statistical} with two intuitions: (1) term frequency saturates so repeating the same identifier does not dominate, and (2) longer documents are length-normalized so short files are not unfairly penalized. This makes BM25 reliable for identifiers and error codes (e.g., \texttt{ERR\_TIMEOUT\_503}) that dense retrievers often miss.
...
\textbf{Example (anchoring the strategy):} When the user asks "Which tests verify REQ-123?", the agent first issues a \texttt{keyword\_search} to anchor the requirement identifier and then calls \texttt{graph\_traverse} to pull its neighboring tests. For a hybrid query like "Login tests with database timeouts", it executes \texttt{hybrid\_search} so that semantic cues ("login") and lexical anchors ("timeout") are fused before drafting a response.

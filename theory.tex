...
Standard text splitting (e.g., every 500 characters) is detrimental in software contexts where maintaining logical integrity is paramount. Instead, \textit{structure-aware splitting} strategies are employed:
\begin{itemize}
...
\subsubsection{Embedding Models (Domain-Specific Vectorization)}
Embedding models transform processed text chunks into numerical vector representations. For technical domains, generic language models often fail to capture the semantic nuance of code or specialized terminology. Therefore, the use of \textit{domain-specific embedding models} (often open-weights models optimized for code, such as those from the BERT or Llama families) is critical. These models are deployed within the secure infrastructure, ensuring data privacy while providing high-fidelity semantic mapping.
...
\begin{itemize}
    \item \textbf{Vector Indices:} Store the high-dimensional embeddings, enabling efficient similarity search (e.g., "Find tests related to authentication failures").
    \item \textbf{Knowledge Graphs (KGs):} Explicitly model the relationships between entities (e.g., \texttt{Requirement-A} $\xrightarrow{verifies}$ \texttt{Test-B} $\xrightarrow{covers}$ \texttt{Function-C}).
    \item \textbf{GraphRAG:} A hybrid approach where vector embeddings are stored as properties within the nodes of a Knowledge Graph. This allows for powerful composite queries: "Find all test cases that verify requirements semantically similar to 'Login Security' AND are linked to the 'AuthService' module."
\end{itemize}

\subsection{Context Engineering}
\label{sec:context-engineering}
Retrieving the right data is only half the battle; presenting it effectively to the LLM is equally critical. \textit{Context Engineering} involves optimizing the input prompt to maximize the model's reasoning performance.
\begin{itemize}
    \item \textbf{Context Window Management:} Selecting the most relevant chunks to fit within the model's token limit without diluting the signal with noise.
    \item \textbf{Prompt Structuring:} Organizing retrieved artifacts logically (e.g., "Relevant Code," "Related Requirements," "History of Failures") to help the model distinguish between different types of evidence.
    \item \textbf{Data Quality:} Ensuring that retrieved context is clean, de-duplicated, and up-to-date to prevent "garbage in, garbage out" scenarios.
\end{itemize}

...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Deterministic Guardrails:} Use rule-based logic to enforce strict compliance. For test scope analysis, this includes verifying that suggested test files actually exist in the repository before recommendation, or enforcing PII redaction to mask sensitive user IDs in bug reports before they are processed by the model.
...
Effective RAG and Agentic systems are not merely about retrieving data, but about \textit{adapting} it to maximize the LLM's reasoning capabilities. Context Engineering represents a systematic framework for this adaptation, treating context not as a static buffer but as an evolving playbook for the agent.
...
\begin{itemize}
    ...
    \item \textbf{Delimited Evidence:} Using clear XML-style tags (e.g., \texttt{<code_snippet>}) to demarcate external data from internal logic, preventing the model from confusing retrieved evidence with system instructions.
\end{itemize}
...
\textbf{Connection to Integration:} BM25 provides the lexical precision tool that the agent exposes as \texttt{keyword_search()} (Section \ref{sec:tool-mediated-integration}). This tool is critical when the agent detects specific, precise identifiers (e.g., error codes, exact function names) in the user's query (Section \ref{sec:query-analysis}), ensuring that structurally relevant but semantically distant artifacts are retrieved effectively.

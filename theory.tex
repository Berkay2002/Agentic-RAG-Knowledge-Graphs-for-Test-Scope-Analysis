...
In the context of software repositories, sparse retrieval is indispensable for locating exact string matches. Unlike natural language, source code relies on precise identifiers. A developer searching for a specific error constant (e.g., \texttt{ERR\_TIMEOUT\_503}) or a unique function signature requires an exact lexical match, which dense retrievers often fail to capture due to tokenization or semantic abstraction. The industry standard algorithm is \textbf{BM25} (Best Matching 25), a probabilistic relevance framework that improves upon TF-IDF \cite{robertson2009probabilistic, robertson1994approximations}.
\begin{itemize}
    \item \textbf{Mechanism:} BM25 scores documents based on the frequency of query terms, penalizing common words \cite{jones1972statistical} and normalizing for document length.
    \item \textbf{Strengths:} Extremely effective for exact matches, proper nouns, and specific code identifiers (e.g., function names like \texttt{processUserPayment}).
    \item \textbf{Weaknesses:} "Vocabulary mismatch problem" – it misses relevant documents that use different words to describe the same concept.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \textbf{The ACE Cycle:} Building on this, advanced systems employ the ACE cycle \cite{zhang2025agenticcontextengineeringevolving} for self-improvement: \textit{Generation} (of the plan), \textit{Reflection} (critiquing the plan/result), and \textit{Curation} (saving the lesson to memory).
    \end{enumerate}

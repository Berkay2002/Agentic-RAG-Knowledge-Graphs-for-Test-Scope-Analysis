...
To rigorously assess the performance of the proposed retrieval system, standard information retrieval metrics are employed.

\subsection{Precision and Recall}
\label{sec:precision-recall}
\begin{itemize}
    \item \textbf{Precision:} The fraction of retrieved items that are relevant. In test scope analysis, high precision means the system does not waste the engineer's time with irrelevant test suggestions.
    \item \textbf{Recall:} The fraction of relevant items that are retrieved. High recall is critical to ensure that no necessary tests are missed, which could lead to undetected bugs.
    \item \textbf{F1-Score:} The harmonic mean of precision and recall, providing a balanced single metric.
\end{itemize}

\subsection{Rank-Aware Metrics}
\label{sec:rank-aware-metrics}
Since the system provides a ranked list of recommendations, the order matters.
\begin{itemize}
    \item \textbf{Precision@k:} Measures precision at a specific cutoff rank $k$ (e.g., top-5 or top-10 results). This reflects the user experience, as users rarely look beyond the first few results.
    \item \textbf{Recall@k:} Measures how many of the relevant items appear in the top $k$ results.
    \item \textbf{NDCG (Normalized Discounted Cumulative Gain):} A measure of ranking quality that accounts for the position of relevant items. It penalizes highly relevant documents that appear lower in the list, providing a more nuanced view of ranking effectiveness than simple precision or recall.
\end{itemize}